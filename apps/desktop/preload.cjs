...
contextBridge.exposeInMainWorld('mccodeDesktop', {
  ...
  aiSetKey: (key) => ipcRenderer.invoke('mccode:ai-set-key', key),
  aiClearKey: () => ipcRenderer.invoke('mccode:ai-clear-key'),
  aiComplete: (request) => ipcRenderer.invoke('mccode:ai-complete', request),
  aiCancel: (id) => ipcRenderer.invoke('mccode:ai-cancel', id),
